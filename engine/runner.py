...
from engine.data import filter_session, load_csv, parse_dt
from engine.cache import ensure_broker_data, load_cache
from engine.indicator import calculate_pine_replica
from engine.trades import build_trades
from engine.execution import ExecutionManager, enabled_execution_modes
from brokers.instrument_selector import select_nearest_futures, apply_execution_instrument
...
def print_dashboard(cfg: Dict[str, Any], summary: Dict[str, Any], signals: pd.DataFrame, trades: pd.DataFrame) -> None:
    ...
    modes = enabled_execution_modes(cfg)
    omode = exe_cfg.get("mode", "PAPER")
    enabled = bool(exe_cfg.get("enabled", False))
    print(f"Execution Engines : {', '.join(modes)}")
    print(f"Execution Mode    : {omode} / {'enabled' if enabled else 'disabled'}")
...
def _latest_closed_trade_key(trades: pd.DataFrame) -> tuple | None:
    if trades is None or trades.empty:
        return None
    r = trades.iloc[-1]
    return (str(r.get("entry_time")), str(r.get("exit_time")), str(r.get("exit_signal")), str(r.get("points")))


def print_trade_closed_summary(cfg: Dict[str, Any], trades: pd.DataFrame) -> None:
    if trades is None or trades.empty:
        return
    r = trades.iloc[-1]
    points = float(r.get("points", 0.0) or 0.0)
    pnl = float(r.get("pnl_value", 0.0) or 0.0)
    exit_signal = str(r.get("exit_signal", ""))
    status = "MAE EXIT" if exit_signal.startswith("MAE_EXIT") else ("PROFIT" if points >= 0 else "LOSS")
    color = Ansi.YELLOW if status == "MAE EXIT" else _pnl_color(points)
    print("\n" + _color("═" * 72, Ansi.BLUE))
    print(_color("TRADE CLOSED", color + Ansi.BOLD))
    print(_color("═" * 72, Ansi.BLUE))
    print(f"Entry Signal   : {r.get('entry_signal')}")
    print(f"Entry Time     : {r.get('entry_time')}")
    print(f"Entry Price    : {r.get('entry_price')}")
    print(f"Exit Signal    : {r.get('exit_signal')}")
    print(f"Exit Time      : {r.get('exit_time')}")
    print(f"Exit Price     : {r.get('exit_price')}")
    print(f"Position       : {r.get('position')}")
    print(f"Points         : {_color(_fmt_points(points), _pnl_color(points) + Ansi.BOLD)}")
    print(f"PnL            : {_color('₹' + f'{pnl:,.2f}', _pnl_color(pnl) + Ansi.BOLD)}")
    print(f"Trade Status   : {_color(status, color + Ansi.BOLD)}")
    print(_color("═" * 72, Ansi.BLUE) + "\n")
...
def print_live_dashboard(
    ...
    print("\n" + _color("=" * 72, Ansi.BLUE))
    print(_color("SVMKR UT HMA ORB CHOP-NO-ADX LIVE STATUS", Ansi.CYAN + Ansi.BOLD))
    print(_color("=" * 72, Ansi.BLUE))
    ...
    print(_color("-" * 72, Ansi.BLUE))
    print(_color("CURRENT STATUS", Ansi.CYAN + Ansi.BOLD))
    pos_color = Ansi.GREEN if current_position == "LONG" else Ansi.RED if current_position == "SHORT" else Ansi.YELLOW
    print(f"Current Position  : {_color(current_position, pos_color + Ansi.BOLD)}")
    if current_entry_price is not None:
        print(f"Current Entry     : {current_entry_time} @ {current_entry_price}")
        print(f"Open Points       : {_color(_fmt_points(open_points), _pnl_color(open_points) + Ansi.BOLD)}")
        print(f"Open PnL          : {_color('₹' + f'{open_pnl:,.2f}', _pnl_color(open_pnl) + Ansi.BOLD)}")
        if mae_enabled and mae_points is not None:
        ...
        print("Open PnL          : 0.00")
    if last_signal:
        sig = str(last_signal.get('signal'))
        sig_color = Ansi.GREEN if sig == 'BUY' else Ansi.RED if sig == 'SELL' else Ansi.YELLOW
        print(f"Last Signal       : {_color(sig, sig_color + Ansi.BOLD)} @ {last_signal.get('price')} on {last_signal.get('datetime')}")
    ...
    if show_signal_results:
        print(_color("-" * 72, Ansi.BLUE))
        print(_color("LAST 10 SIGNAL RESULTS", Ansi.CYAN + Ansi.BOLD))
...
def _reconstruct_from_cache(cfg: Dict[str, Any], now: pd.Timestamp, out_dir: Path, prefix: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    """Reconstruct state from local candle cache only.

    This is used for immediate startup display and as a safe fallback when the
    broker historical API is temporarily rate-limited. It never calls AngelOne.
    """
    instr = cfg.get("instrument", {}) or {}
    df = load_cache(cfg)
    if df is None or df.empty:
        raise RuntimeError("Local candle cache is empty; cannot reconstruct cached state.")
    start_dt = _warmup_start(now, cfg)
    df = df[(df["datetime"] >= start_dt) & (df["datetime"] <= now)].copy()
    df = filter_session(df, instr.get("session_start"), instr.get("session_end"))
    interval_minutes = int(instr.get("timeframe_minutes", 5) or 5)
    df = _drop_unclosed_last_candle(df, interval_minutes, now)
    if df.empty:
        raise RuntimeError("Local candle cache has no candles for current warmup window.")
    calc = calculate_pine_replica(df, cfg)
    signals, trades, summary = build_trades(calc, cfg)
    write_outputs(out_dir, calc, signals, trades, summary, prefix=prefix)
    return calc, signals, trades, summary
...
def _resolve_execution_instrument_if_needed(cfg: Dict[str, Any], broker=None) -> None:
    ...
    # Option modes need the scrip master cached, but strikes are selected at
    # signal time because CE/PE and strike depend on signal direction and LTP.
    if any(m in {"OPTION_BUYING", "OPTION_SELLING"} for m in modes) and auto_select:
        from brokers.instrument_selector import load_scrip_master
        rows = load_scrip_master(cfg, force=bool(selector.get("force_refresh", False)))
        print(f"Instrument list loaded: {len(rows)} contracts")
        opt_modes = [m for m in modes if m in {"OPTION_BUYING", "OPTION_SELLING"}]
        print(f"Option execution modes ready: {', '.join(opt_modes)}. Strikes will be selected on each fresh signal.")

    # Futures mode still resolves the active futures contract at startup.
    if "FUTURES" not in modes:
        return
...
def run_live(cfg: Dict[str, Any]) -> None:
    ...
    last_reported_trade_key: Optional[tuple] = None
    last_recommendation_key: Optional[tuple] = None


    def maybe_print_execution_recommendation(summary: Dict[str, Any], reason: str) -> None:
        nonlocal last_recommendation_key
        last_signal = summary.get("last_signal") or {}
        key = (
            str(last_signal.get("datetime")),
            str(last_signal.get("signal")),
            str(summary.get("current_position")),
            str(summary.get("current_entry_price")),
        )
        if key == last_recommendation_key:
            return
        if str(summary.get("current_position", "FLAT")).upper() == "FLAT":
            return
        execution_manager.print_recommendation_for_summary(summary, reason=reason)
        last_recommendation_key = key
    ...
    while True:
        ...
        if state in {"CLOSED_WEEKEND", "CLOSED_HOLIDAY"}:
            ...
            last_reported_trade_key = _latest_closed_trade_key(trades)
            if not execution_started:
                execution_manager.startup_sync(summary)
                execution_started = True
            maybe_print_execution_recommendation(summary, "CLOSED_DAY_CURRENT_SIGNAL")
        ...
        if state in {"PRE_MARKET", "OPENING_RANGE"}:
            if last_dashboard_state != state:
                ...
                if not execution_started:
                    execution_manager.startup_sync(summary)
                    execution_started = True
                maybe_print_execution_recommendation(summary, "PREMARKET_CURRENT_SIGNAL")
        ...
        if state == "POST_MARKET":
            print("Market is closed for the day now. Preparing graceful shutdown...")
            # After 15:30 AngelOne may throttle/deny historical candle calls.
            # A post-market refresh is useful, but it must never crash the engine.
            # Try broker first, then fall back to the latest local cache/dashboard state.
            try:
                _, signals, trades, summary = _reconstruct_from_broker(cfg, broker, now, out_dir, prefix="live_postmarket")
                postmarket_source = "broker final refresh"
            except Exception as exc:
                if _is_rate_limit_error(exc):
                    print("AngelOne rate limit hit during post-market refresh. Using cached latest state for shutdown.")
                else:
                    print(f"Post-market refresh failed: {exc}. Using cached latest state for shutdown.")
                try:
                    _, signals, trades, summary = _reconstruct_from_cache(cfg, now, out_dir, prefix="live_postmarket_cache")
                    postmarket_source = "cached state"
                except Exception as cache_exc:
                    print(f"Cached post-market state unavailable: {cache_exc}. Shutting down without final dashboard refresh.")
                    signals, trades, summary = pd.DataFrame(), pd.DataFrame(), {"current_position": "UNKNOWN", "open_points": 0.0, "open_pnl": 0.0}
                    postmarket_source = "unavailable"
            msg = _session_wait_message(now, target, "Next trading session")
            suffix = "\nMarket is closed for the day now. Engine is shutting now." if auto_exit else "\nMarket is closed for the day now. Engine will remain alive and wait for the next session."
            print_live_dashboard(
                cfg,
                summary,
                signals,
                trades,
                market_status=f"{status_text} / {postmarket_source}",
                exit_message=msg + suffix,
                show_signal_results=not startup_signal_results_shown,
            )
            startup_signal_results_shown = True
            last_reported_trade_key = _latest_closed_trade_key(trades)
            if not execution_started:
                execution_manager.startup_sync(summary)
                execution_started = True
            maybe_print_execution_recommendation(summary, "POSTMARKET_CURRENT_SIGNAL")
            if auto_exit:
                return
            try:
        ...
        if last_dashboard_state != "LIVE":
            ...
            # Show immediate reconstructed status from local cache before any
            # broker historical API fetch/backoff. This avoids a blank console
            # for several minutes if AngelOne throttles the first live fetch.
            if not startup_signal_results_shown:
                try:
                    _, cached_signals, cached_trades, cached_summary = _reconstruct_from_cache(cfg, now, out_dir, prefix="live_startup_cache")
                    print_live_dashboard(
                        cfg,
                        cached_summary,
                        cached_signals,
                        cached_trades,
                        market_status=f"{status_text} / cached startup state",
                        show_signal_results=True,
                    )
                    startup_signal_results_shown = True
                    last_reported_trade_key = _latest_closed_trade_key(cached_trades)
                    if not execution_started:
                        execution_manager.startup_sync(cached_summary)
                        execution_started = True
                    maybe_print_execution_recommendation(cached_summary, "STARTUP_CURRENT_SIGNAL")
                except Exception as exc:
                    print(f"Startup cache reconstruction unavailable: {exc}")
        ...
        try:
            ...
            if latest_closed is not None and latest_closed != last_processed_candle:
                ...
                current_trade_key = _latest_closed_trade_key(trades)
                if current_trade_key is not None and current_trade_key != last_reported_trade_key:
                    print_trade_closed_summary(cfg, trades)
                    last_reported_trade_key = current_trade_key
                ...
                maybe_print_execution_recommendation(summary, "CANDLE_CLOSE_SIGNAL")
